	...
	NumberFor<Block>: BlockNumberOps,
{
	...
	fn prevoted(&self, round: RoundNumber, prevote: Prevote<Block>) -> Result<(), Self::Error> {
		...
		let mut report_prevote_metrics = move |prevote: &Prevote<Block>| {
			telemetry!(telemetry; CONSENSUS_DEBUG; "afg.prevote_issued";
				"round" => round,
	...
	fn precommitted(
		...
		let mut report_precommit_metrics = move |precommit: &Precommit<Block>| {
			telemetry!(telemetry; CONSENSUS_DEBUG; "afg.precommit_issued";
				"round" => round,
	...
	Client: crate::ClientForGrandpa<Block, BE>,
{
	...
	let update_res: Result<_, Error> = client.lock_import_and_run({
		let mut telemetry = telemetry.clone();
		let mut authority_set = authority_set.clone();
		...
		move |import_op| {
			...
			fn notify_justification<Block: BlockT>(
				...
				if let Some(sender) = justification_sender {
					if let Err(err) = sender.notify(justification) {
						warn!(
							target: "afg",
							"Error creating justification for subscriber: {:?}",
							err,
						);
					}
				}
			...
			telemetry!(telemetry; CONSENSUS_INFO; "afg.finalized_blocks_up_to";
			...
			let new_authorities = if let Some((canon_hash, canon_number)) = status.new_set_block {
				...
				telemetry!(telemetry; CONSENSUS_INFO; "afg.generating_new_authority_set";
